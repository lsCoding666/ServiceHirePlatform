    ...
    public Okhttp3Client() {
        ...
        this.okHttpClient = new OkHttpClient.Builder()
                ...
                .addNetworkInterceptor(chain -> {
                    ...
                    if (!SpUtil.getBoolean(FinalDatas.IS_LOGIN) && !originalResponse.headers("Set-Cookie").isEmpty()) {
                        //noinspection AlibabaUndefineMagicConstant
                        SpUtil.setString(FinalDatas.COOKIE, originalResponse.headers("Set-Cookie").get(0));
                    }
                ...
                .addNetworkInterceptor(chain -> {
                    Request request;
                    if (!SpUtil.getString(FinalDatas.COOKIE).isEmpty()) {
                        request = chain.request().newBuilder().addHeader("Cookie",
                                SpUtil.getString(FinalDatas.COOKIE)).build();
                    } else {
